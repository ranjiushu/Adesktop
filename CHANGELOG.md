...
### 贡献授权与第三方来源台账（2026-09-26）

- **新增 `CONTRIBUTING.md`**：提交前门禁 / 分支约定 / 贡献授权声明——提交贡献即向维护者
  授予非独占、永久、不可撤销、可再许可的著作权许可（含以与 GPL-3.0 并行的其他条款分发），
  以保证后续分许可方式（如商业授权）不受外部贡献阻挡；含第三方内容时须标注来源并同步台账。
- **新增 `docs/third-party.md`**：非自研内容的来源与许可台账（MDI 图标数据、androidx、
