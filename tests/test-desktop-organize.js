...
// ── 2. 竖屏列优先（从上到下）──
// 视口 412×700、zoom 1、相机 (0,0)（x/y = 可见区域左上角语义）：可见区域 0..412 × 0..700，cols=4, rows=6
const vp = O.organize(items, 412, 700, { x: 0, y: 0, zoom: 1, rotation: 0 })
check(vp.length === items.length, 'organize 输出全部条目')
check(vp[0].x === 0 && vp[0].y === 0, '竖屏第一项 = 可见区域左上角 (0,0)（相机 x/y 即左上角）')
...
check(hp[0].x === 0 && hp[0].y === 0, '横屏第一项 = 左上角 (0,0)')
