...
- `render()` 只为**当前视口内**的图片/视频图标派发缩略图请求（视口外扩半屏预取，
  `THUMB_OVERSCAN`）——离屏条目不生成缩略图，省桥层采样解码/视频首帧提取与 base64 内存：
  进 200 张图的目录只请求首屏那几十个，不再一次性排满整目录的任务。
- 平移/缩放/滚动/Home 飞行后按需补齐：相机变化经 `scheduleVisibleThumbs` 节流
  （`THUMB_REFILL_MS`；连续变化每窗口至多一轮）派发新进入视口的条目。
- 同一路径只派发一次（`_thumbRequested`）；**重新渲染会重建该集合**——DOM 元素每次渲染
  都是新的，重建保证「等待中的缩略图落地时元素已被替换」能由下一次渲染重新派发
  （Thumbnail 侧 pending 合并 waiter），避免类型图标卡住。
- 缩略图未就绪时显示类型图标占位（渐进式替换，不阻塞布局与手势）。
