...
本文件是 Desktop **文件操作语义**的权威清单：每个操作的前置条件、成功/失败/取消后的
...
- [P0] 已知缺陷：批量操作中，用户取消只中止**当前正在传输的文件**；JS 批量链
  `cancelSent` 后**仍继续调度剩余项**，且桥层每次 `move()/copy()` 开头把取消标志
  **重置为 false** → 剩余项全部照常执行。取消后用户看到「失败 N 项」但文件其实被移动了。
  （修复见第 ④ 步：取消后停止调度剩余项，结束态报「已取消」而非「失败」。）
...
| 取消 | 当前文件中止 + 清理半成品；[P0] 剩余项仍会继续执行（见 1.3） |
...
| 6 | **取消后停止调度剩余项（3 以后不再启动）** | — | [P0] **待补（第 ② 步，先红后绿）** |
