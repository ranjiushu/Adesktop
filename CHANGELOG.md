...
- **实体基本性质**：点击 Viewer = 选中（与文件选中态绑定，accent 选中视觉）；
  长按/拖动 = 移动实体位置（世界坐标位移，取消还原）；点击不穿透
- **全屏 = 独立新页面**（#viewer-fs-page，fixed 覆盖全视口，FAB 隐藏，页头返回按钮）：
  退出（返回键 / 页头按钮）回到原页面状态——桌面空间回画布实体（内容与位置保留），
  folder 容器关闭回目录；pushState 支持系统返回键
- **Morph FAB 预览焦点模式**：Viewer 打开时选中态操作栏只显示「全屏预览」「关闭预览」
  （文件操作隐藏）；Viewer 关闭后恢复 打开/复制/剪切/重命名/取消选择
