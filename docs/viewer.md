...
- **文件锁定（Windows 式）**：被 Viewer 打开的文件进入锁定状态（图标 🔒 标记）——
  ...
  **图标与预览窗口单向锚定**（防分家/重叠，图标 = 网格真相锚点）：拖动锁定文件图标 →
  Viewer 窗口实时跟随（含 drop 网格吸附后的最终落位）；**Viewer 窗口拖动/媒体自适应
  不带动图标**——窗口自由浮动（用户可把预览拖到别处看内容，图标保持网格位置），
  仅会话恢复时（restoreViewers）图标贴窗对齐一次防历史分家数据。
  **整理桌面跳过锁定文件**（其余排布让开其占位格）；其它文件拖动不能
