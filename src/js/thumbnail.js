 ...
 *
 * 职责：
 *   - canThumbnail(kind)：判定类型是否可生成缩略图（图片；视频预留）
 *   - request(path, name, kind, onReady, onFallback)：渐进式获取缩略图 URI
 *       · 命中缓存（ready）→ 立即 onReady(uri)
 *       · 失败缓存（failed）→ 立即 onFallback()
 *       · 未命中 → 先回退类型图标，异步 resolveUri + 解码验证，成功后 onReady
 *       · 同一路径并发请求合并（pending 去重），避免重复 resolveUri / 解码
...
App.Thumbnail = (function () {
  ...
  // 本次可缩略图类型：图片（WebView 原生解码，含 SVG 渲染预览 / GIF 首帧）。
  // 视频缩略图待桥层 MediaMetadataRetriever 提取帧后在此放行 'video'。
